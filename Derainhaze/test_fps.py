...
from models.PiM_multioutput import UMamba_full_fps
...
class FPSBenchmark():
            ...
            if i >= self.warmup_num:
                ...
                if (i + 1) % self.log_interval == 0:
                    ...
                    print(
                        ...
                        f'FPS: {fps:.6f} img/s, '
                        f'Times per image: {1000 / fps:.6f} ms/img',
                        flush=True,
                    )
                else:
                    pass
    ...
    def repeat_measure_inference_speed(self):
        ...
        if self.repeat_num > 1:
            fps_list_ = [round(fps, 4) for fps in fps_list]
            times_pre_image_list_ = [round(1000 / fps, 4) for fps in fps_list]
            mean_fps_ = sum(fps_list_) / len(fps_list_)
            mean_times_pre_image_ = sum(times_pre_image_list_) / len(
                times_pre_image_list_)
            print(
                f'Overall FPS: {fps_list_}[{mean_fps_:.6f}] img/s, '
                f'Times per image: '
                f'{times_pre_image_list_}[{mean_times_pre_image_:.6f}] ms/img',
                flush=True,
            )
            
            std_fps_ = statistics.stdev(fps_list_)
            std_times_pre_image_ = statistics.stdev(times_pre_image_list_)
            print(
                f'STD FPS: {std_fps_:.6f} img/s, '
                f'STD Times per image: {std_times_pre_image_:.6f} ms/img',
                flush=True,
            )
...
if __name__ == '__main__':
    model_restoration = UMamba_full_fps()

    fps_benmark = FPSBenchmark(model=model_restoration,
                            input_size=(3, 480, 720),
